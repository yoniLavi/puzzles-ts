...
| **the agent's LSP `findReferences`, cold** | issued while the language server was still loading the project, it returned **2 references in 1 file** for `tierNames`, with no error or warning | the identical query a minute later: 72 references across 32 files, every tiered game (`weigh-a-semantic-code-server`). a second run answered 2, 2, 2 across 3.6 s, so a wait-until-stable loop is fooled too. `npm run refs` builds the whole program before answering, and `tsgo --lsp` answered in full on its first query in both runs |
