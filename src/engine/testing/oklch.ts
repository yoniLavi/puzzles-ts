/**
 * OKLCH conversion for the engine's color tests — deliberately a *different*
 * implementation from the one under test.
 *
 * `colors.ts` inlines the OKLab→sRGB matrices rather than importing a color
 ...
 * maths and a real library agree, so `colors.test.ts` pins it — and the pin is
 ...
 * silently change what the engine's color tests assert. (The other half of the
 ...
 * hue in [0, 360] — or NaN for an achromatic color, which is why any distance
...
/** Below this chroma a color has no meaningful hue, and NaN hues appear. */
