...
describe("FlipDsf", () => {
  ...
  it("matches a brute-force parity reference under random merges", () => {
    // The per-op cost is the O(n²) full-matrix re-check below, so wall time
    // scales as n²·ops. The parity property is n-independent — 16 elements
    // under 40 consistent flip-merges already build deep multi-way equivalence
    // classes with inversions — so a larger n buys cost, not rigor.
    const n = 16;
