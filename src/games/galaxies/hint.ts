  ...
  switch (firing.kind) {
    ...
    case "mirrorWall": {
      ...
      return atBoardEdge(s, firing.from)
        ? `The two shaded cells are partners across the ${dot}, and one of them is up against the edge of the board — so the other must be walled off on the matching side.`
        : `A galaxy looks the same turned 180° about its dot: the two shaded cells are partners across the ${dot}, so the marked wall beside one must be matched beside the other.`;
    }
    case "enclosed": {
      const lead =
