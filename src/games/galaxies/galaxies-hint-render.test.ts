...
describe("a displayed hint reaches the canvas", () => {
  it("an association fills its cells and rings the dot it points at", () => {
    ...
    // One solid COL_HINT fill per claimed cell — plus nothing else, because
    // an association step draws no wall.
    const fills = rects(recording.ops, COL_HINT).filter(
      (o) => o.op === "rect" && o.w > 4 && o.h > 4,
    );
    expect(fills).toHaveLength(hl.targets.length);
