...
function narrate(f: LightupFiring, hl: LightupHint): string {
  ...
  switch (f.reason.kind) {
    case "forcedLight": {
      ...
      return "The ringed square is still dark and only this square can still light it, so this one must hold a bulb.";
    }
    case "clueSatisfied": {
      const { n } = f.reason;
      if (n === 0) {
    ...
    case "clueSaturated": {
      ...
      return `The highlighted clue still needs ${need} more bulbs and has exactly ${need} free neighbors left, so each must be a bulb.`;
    }
    case "discountUnlit": {
    ...
    case "discountClue":
      return "The highlighted clue needs a bulb in one of the shaded squares, and a bulb here would leave each of them lit or beside a full clue, so this square must be crossed out.";
  }
}
