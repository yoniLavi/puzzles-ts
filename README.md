...
* src/assets/icons and src/assets/puzzles are generated by host-native
  build scripts ([see below](#building-puzzles)). They're not included
  in the repo.
...
The build has two halves: host-native shell scripts produce the puzzles' wasm
and icon assets; vite bundles the TypeScript PWA around them.
...
All native build dependencies (Emscripten, GTK+3, ImageMagick, halibut, jq,
oxipng, etc.) are listed in the repo-root [Brewfile](Brewfile). On macOS or
Linuxbrew:
...
environment variables — see the comments at the top of each.
