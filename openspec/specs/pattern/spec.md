...
`interpretMove` SHALL reproduce upstream's input: a left/right/middle press
begins a drag setting the target state (`Full` / `Empty` / `Unknown`, with the
...
cell in it would change** (otherwise a UI update). Keyboard cursor movement with
the control/shift modifiers SHALL set cells to `Empty` / `Full` / `Unknown` via
the same rectangle move, and the cursor-select keys SHALL cycle a cell's state.
Immutable cells SHALL never be overwritten.

#### Scenario: A drag that changes cells emits a move

- **WHEN** the player left-drags across cells not all already `Full`
- **THEN** release emits a `fill` move setting that line of cells to `Full`
