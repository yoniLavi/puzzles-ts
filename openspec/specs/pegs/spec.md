...
The Pegs `colours()` method SHALL derive its background, highlight, and lowlight colours from the shared `mkhighlight` helper in `src/native/engine/colour-mkhighlight.ts`, with no local copy of the derivation.
