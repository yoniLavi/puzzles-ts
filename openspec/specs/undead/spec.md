...
The solver SHALL combine a per-sightline candidate-intersection pass (iterated to a
fixpoint) with a whole-grid brute-force uniqueness search, over an odometer of the
`{ghost, vampire, zombie}` choice at each cell constrained by the current candidate
set. `newDesc` SHALL generate a grid of random mirrors and monster cells (rejecting
grids that are too sparse, too dense, or have an over-long sightline), seed
unique-solution sightlines until a difficulty-dependent fraction of the grid is
determined, fill the remainder with random monsters, and accept the board only when
the graded solver solves it at the requested difficulty (regenerating otherwise).
Every generated board SHALL be uniquely solvable.

#### Scenario: Generated board is unique and on-difficulty

- **WHEN** `newDesc` returns a board for a given size and difficulty
- **THEN** the brute-force solver finds exactly one solution
- **AND** the graded solver classifies it at the requested difficulty
