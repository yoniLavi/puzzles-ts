...
- [x] 1.1 `scripts/new-game-port.sh bricks` to stamp `src/native/games/bricks/`
...
- [x] 2.1 `BricksParams { w, h, diff }` (`diff` = Easy/Normal/Tricky);
...
- [x] 2.2 `validateParams`: `w ≥ 2`, `h ≥ 2`, `diff < DIFFCOUNT`.
- [x] 2.3 `paramConfig` (Width, Height, Difficulty choices) with keys matching the C
      config slugs (playbook §3.4); numeric `set` via `parseConfigInt`, difficulty
      as a choice list.
- [x] 2.4 Presets — the six upstream presets (`7×6` and `10×8` × Easy/Normal/Tricky);
      `describeParams` emits the keys `augmentation.ts` reads (playbook §3.4).
- [x] 2.5 Hex geometry helpers (design D2): `gridSize(params)` →
      ...
      neighbour counts.
- [x] 2.6 State: immutable `grid` (packed `Uint8Array`/`Uint16Array` cell field —
...
- [x] 2.7 Desc codec (design D5): decimal-digit numbers with `_` separator between
      ...
      byte-match surface.
- [x] 2.8 `validateDesc`: reject too-many/too-few decoded cells (distinguishing
      which) and a clue `> 7`; bounds skipped against the geometry mask.

## 3. The solver (contradiction + bounded lookahead)

- [x] 3.1 `validate(grid, strict)` → discriminated `COMPLETE | UNFINISHED | INVALID`
...
- [x] 3.2 `solverTry` (single-cell contradiction = Easy) and `solverRecurse` (bounded
...
- [x] 3.3 Tier-1 tests: a hand-built board solves to the unique solution; an
      over-constrained board reports `INVALID`; Easy stops where Normal continues.

## 4. The generator

- [x] 4.1 `fillGrid` (bottom-up shade/unshade under gravity + no-three-run +
...
- [x] 4.2 `removeNumbers`: one `shuffle` of cell order, blank each numbered cell
      ...
      (byte-match surface).
- [x] 4.3 `newDesc` emitting the run-length desc; no `aux` (the solver re-derives the
      solution from the desc, playbook §3.6).
- [x] 4.4 Tier-1: every preset and a small size sweep generate a board that solves to
      `COMPLETE` and round-trips through the codec.

## 5. Input, moves and completion

- [x] 5.1 `BricksMove` discriminated union (`{ kind: "paint"; cells } | { kind:
      "solve"; grid }`), not a move string (design D6).
- [x] 5.2 `interpretMove` paint phases (design D6): grab (colour cycle by
...
- [x] 5.3 Keyboard cursor (design D6): hex-aware up/down (orthogonal↔diagonal
...
- [x] 5.4 `executeMove`: apply each painted cell (only where playable), set
...
- [x] 5.6 `textFormat` — the ASCII board (`board_text_format`); `canFormatAsText`
      static `true` (design D9). No statusbar (`wants_statusbar` false).

## 6. `findMistakes`

- [x] 6.1 `findMistakes(state)` runs the D3 validity pass and returns the offending
...
- [x] 7.1 Palette in C enum order (`COL_MIDLIGHT`/`COL_LOWLIGHT`/`COL_HIGHLIGHT` via
      ...
      §3.3 — the app owns it).
- [x] 7.2 `computeSize`/`setTileSize`: the sheared-hex size (`params.w*ts + ts/2`
      wide, `params.h*ts` tall, `BORDER = 0` under `NARROW_BORDERS` — checked, not
      assumed) and the row offsets (`game_set_offsets`).
- [x] 7.3 `redraw`: per-cell rendering with the per-row `tilesize/2` shear, square
...
- [x] 7.4 Live rule-violation overlay (design D7): the three-in-a-row bar
...
- [x] 7.5 The completion flash over `FLASH_TIME` (five `FLASH_FRAME` intervals,
      shaded cells blink to empty). No slide/interpolation animation (`animLength`
      is `0`).
- [x] 7.6 Tier-2.5 render-scenario tests + snapshots: a numbered board, a shaded-run
      frame, a three-in-a-row error frame, and a completion-flash frame.

## 8. Differential

- [x] 8.1 `puzzles/auxiliary/bricks-trace.c` on the established pattern; add its
      `cliprogram()` line.
- [x] 8.2 Fixture matrix: every preset plus a size sweep (design D8), each seed
      dumping the generated desc.
- [x] 8.3 `bricks-differential.test.ts`: TS `newDesc` reproduces the C desc
...
- [x] 9.3 `openspec validate add-bricks-ts-port --strict`.
- [x] 9.4 Dev-verify in the browser: paint (click + drag, both buttons), keyboard
      cursor across rows, live errors, Check & Save hard-block on a mistake, Solve,
      completion flash, Custom params.
- [x] 9.5 Update `docs/porting/game-port-playbook.md` (the bespoke hex-shear geometry
      + coordinate conversion, live-error-plus-findMistakes sharing one validity
      pass).

## 10. Stage 2 — on owner acceptance only

- [x] 10.1 Add `TS_PORTED` to the `puzzle(bricks …)` entry in
      `puzzles/unreleased/CMakeLists.txt` (and drop `solver(bricks)`).
- [x] 10.2 Delete `puzzles/unreleased/bricks.c`.
- [x] 10.3 `rm -rf build/wasm/` and rebuild — bricks in the catalog, no `bricks.wasm`
