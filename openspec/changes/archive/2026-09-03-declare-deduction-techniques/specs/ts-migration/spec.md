# ts-migration — delta for declare-deduction-techniques

The single substantive edit is the parenthetical *"the highest rung used is the
difficulty grade"*, which this change makes false: a game may declare several
techniques on one tier (Unruly declares five across three), so the grade is the
highest **tier** that fired and never a technique's position. The surrounding
vocabulary moves from "rung" to "technique" with it. Every scenario is
