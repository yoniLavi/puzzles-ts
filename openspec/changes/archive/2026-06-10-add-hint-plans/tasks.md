...
- [x] 1. Engine contract (`src/native/engine/game.ts`): add `HintStep` and
...
- [x] 2. Midend (`src/native/engine/midend.ts`): store the plan; current-step
...
- [x] 3. Engine tests (`midend.test.ts`): migrate the fake game to the new
...
- [x] 4. Sixteen search (`src/native/games/sixteen/index.ts`): reconstruct the
...
- [x] 5. Sixteen narration: extract `narrateStep` and build `HintStep[]` by
...
- [x] 6. Sixteen `hintKeepTrack`: return verdicts; adjust `step.move` delta on
  partial progress (`onTrack`).
- [x] 7. Sixteen tests: migrate hint-shape assertions; switch playthrough
