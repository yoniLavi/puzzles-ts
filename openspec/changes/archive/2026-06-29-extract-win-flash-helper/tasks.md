...
- [x] 1.1 `engine/flash.ts` `winFlash(from, to, flashTime)` — `flashTime` on a fresh,
  un-cheated unsolved→solved transition, else `0`; reads `completed`/`cheated`
  structurally. Unit test (the four state combinations).
- [x] 1.2 Delegate `flashLength` in the games whose flash is exactly the canonical shape
