...
- [x] 1.1 `scripts/new-game-port.sh ascent` to stamp `src/native/games/ascent/`
...
- [x] 2.1 `AscentParams { w, h, diff, mode, removeends, symmetrical }`;
...
- [x] 3.1 Encode (`new_game_desc` tail, `ascent.c:1740-1793`): number runs
...
- [x] 3.2 Decode (`new_game`, `ascent.c:1836`): place numbers + immutable bits,
...
- [x] 4.3 `ascentSolve(puzzle, diff, scratch)` — the tiered fixpoint loop
...
- [x] 4.4 Tier-1: each preset's solution is unique and reached at exactly its
      difficulty (a Hard board is not solved by the Normal ruleset).

## 5. The generator

- [x] 5.1 `generateHamiltonianPath` (`ascent.c:676`): per-mode wall placement
...
- [x] 5.2 Non-Edges reduction `ascentRemoveNumbers` (`ascent.c:1667`): one
      `shuffle`, then solver-gated blanking honouring `symmetrical` (partner cell)
      and `removeends` (keep 0 and last).
- [x] 5.3 Edges reduction `ascentAddEdges` (`ascent.c:1557`): build the inner↔edge
...
- [x] 6.4 Free-form path drawing: left-drag → `line`, right-click/drag → `clear`;
      ...
      loop).
- [x] 6.5 `executeMove` (`ascent.c:3074`): apply each fragment (reject placing on
      ...
      arrow satisfaction).
- [x] 6.6 `solve()` returns `{ kind: "solve", grid }` from a full-difficulty
      `ascentSolve`; error if unsolved. Test Solve **through a real `Midend`**.
- [x] 6.7 `findMistakes` (design D6): re-solve a clues-only copy to the canonical
      solution, flag placed numbers that contradict it. Tier-1: a wrong number is
      reported, a correct partial board reports none.
- [x] 6.8 `textFormat`/`canFormatAsText` per `game_text_format` (`ascent.c:2023`).

## 7. Rendering

- [x] 7.1 Palette in C enum order (`COL_MIDLIGHT … COL_ARROW`, `ascent.c:33`),
      derived from the app background; `COL_ERROR` pure red. No dark-mode
      luminance adjust (playbook §3.3 — the app owns it).
- [x] 7.2 `computeSize`/`setTileSize`: `NARROW_BORDERS` arm (`BORDER = 0`, D9);
      per-mode half-tile offset via `setOffsets` (`ascent.c:3258`).
- [x] 7.3 `redraw`: per-tile numbers/walls/path segments with an `Int32Array`
      cache key (playbook §3.2). Every drag/typing/error/flash overlay in the diff
      key. Own background fill in `!ds.started`.
- [x] 7.4 Edges arrows (`ascentDrawArrow`, `ascent.c:3375-3438`); endpoint
...
- [x] 7.5 The completion flash (`FLASH_FRAME = 0.03`, `FLASH_SIZE = 4`, the wave
      down the path — `ascent.c:3490-3646`). No move animation (`animLength = 0`).
- [x] 7.6 Tier-2.5 render-scenario tests + snapshots: a fresh board per mode
      (Rect / Hexagon / Edges), a mid-typing frame, an error-shaded frame, and a
      completion-flash frame.

## 8. Differential

- [x] 8.1 `puzzles/auxiliary/ascent-trace.c` on the established pattern
      (`#include "../unreleased/ascent.c"`); add its `cliprogram()` line.
- [x] 8.2 Fixture matrix: all five modes × difficulty tiers, plus `symmetrical`
      and `removeends` variants and a small size sweep (design D7), each seed
      dumping the generated desc.
- [x] 8.3 `ascent-differential.test.ts`: TS `newDesc` reproduces the C desc
      **byte-for-byte** for each fixture — validates generator + solver + codec.

## 9. Registration and stage 1 close-out

- [x] 9.1 Register in `ts-ported-ids.ts` + `games/index.ts` (TS-served). The
...
- [x] 9.2 Behavioural tests: codec round-trip, solver uniqueness per tier,
