...
- [x] 1.1 ~~Factor the white-cell wipe out of `sticksSolveGame`~~ — **not
      needed**: `deduceHintPlan` already *is* the fixpoint, so the recorder is a
      parallel `nextSticksFiring`/`deduceSticksPlan` beside the untouched
      generator solver (Bricks' §5.6a′ shape). Byte-identity by construction
      rather than by proof. Design D2, "D2 resolved".
- [x] 1.2 **Prove the generator is byte-identical** — the differential fixtures
      are the check, and they must not move. A moved fixture means the wipe was
      not merely relocated.
- [x] 1.3 Confirm — do not assume — that a mid-game board needs no cascade
...
- [x] 2.4 Gate every reason allocation on the recorder so the solve path stays
      allocation-for-allocation as it was (the Singles discipline, §3).

## 3. Narration

- [x] 3.1 One sentence per kind: indication → reasoning → conclusion (§2.2),
      necessity voice (`must be`, never `is`/`stays` — §2.1), under the
      300-character ceiling (§2.5).
- [x] 3.2 Name squares by what the player can see — the clue's **number**, not
      "this square" or a bare pronoun (§2.3).
- [x] 3.3 **Re-read every sentence at its degenerate clue values** (§2.7): a
      black clue of 0 and of 4, a length clue of 1. "only", "another" and
      "still" are where these break.
- [x] 3.4 Check each premise actually singles out *this* conclusion (§2.4) —
      if two candidate moves satisfy the stated premise, the sentence names the
      wrong reason.
- [x] 3.5 Keep the rules of the game out of the steps (§2.9) — "a line may
      cover only one number" belongs in `help/games/sticks.md` unless it is the
      technique *this* firing turns on.

## 4. Rendering

- [x] 4.1 Add `COL_HINT` (and any evidence colour) to Sticks' palette — it has
      none today; the palette is `COL_BACKGROUND`/`GRID`/`LINE`/`NUMBER`/
      `ERROR`/`CURSOR`.
- [x] 4.2 **Settle D3 against a rendered frame** — the `COL_HINT` bar, confirmed
      on rendered frames for all three shapes of argument and by owner
      acceptance. Recorded in `design.md` ("D3 settled"). **Also unplanned:**
      upstream's cursor was the same `BLUE` the hint wanted, so it moves to
      `PURPLE` on the Spokes/Subsets precedent — design D3a.
- [x] 4.3 Shade the evidence as an **area**, per kind (§5.2, design D4). For
      `unreachable`, shade exactly the span the `maxSize*` walk covered — an
      approximated span makes the sentence false (§2.3).
- [x] 4.4 Fold the hint bits into the per-tile cache key, or the overlay will
      not repaint (playbook §3.2; `hint-overlay.test.ts` covers the class once
      the game is enrolled).

## 5. Guards

- [x] 5.1 **Enrol in [`testing/hint-games.ts`](../../../src/engine/testing/hint-games.ts)** —
...
- [x] 5.2 Per-game tier-2.5 render scenarios (§8): reach a frame for each of the
      ...
      on the wrong frame.
- [x] 5.3 Validate **grouping** on a *generated* board — and the expectation was
      **wrong**: 21% of firings decide more than one square (mean 1.2, max 5),
      the black-clue rules hardest, so Sticks does emit `continuesPrevious`
      journeys. Design D5, "D5 resolved". Exactly the reason the task said to
      scan seeds rather than read `sticksTry`.
- [x] 5.4 Refusal: Sticks already declares `findMistakes`, so a wrong board
      gets "fix the highlighted mistakes first" with the cells lit, for free
      (§4). Assert it rather than assuming the wiring.
- [x] 5.5 No `refreshHintStep` (design D6) — state the reason in the code so the
      next reader does not have to re-derive it.

## 6. Close out

- [x] 6.1 Spec delta `sticks`; add Sticks to the `ts-engine` hint requirement's
      implementers if that list is maintained.
- [x] 6.2 Update `help/games/sticks.md` — it must teach any vocabulary the hint
      introduces, and a hint and a help page that disagree are worse than either
      alone (§2.8).
- [x] 6.3 Update `docs/porting/hint-authoring.md` with whatever this change
      ...
      beyond Boats.
- [x] 6.4 Full gate green; differential fixtures unmoved (all 13 byte-match);
      owner acceptance given 2026-08-06.
