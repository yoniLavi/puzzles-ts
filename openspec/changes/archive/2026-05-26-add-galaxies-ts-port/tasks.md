...
- [x] 2.1 `src/native/games/galaxies/index.ts`: types
...
- [x] 3.7 Top-level `solver_state(state, maxDiff)`: returns
      `NORMAL` / `UNREASONABLE` / `AMBIGUOUS` / `IMPOSSIBLE` /
      `UNFINISHED` matching upstream's contract.
- [x] 3.8 Solver behavioural tests: hand-crafted positions for each
...
- [x] 4.1 `newDesc(p, rng)`: structural port of upstream's
      `new_game_desc` — place dots, validate via solver at requested
      difficulty in the retry loop, emit the desc bitmap.
- [x] 4.2 `validateDesc`: dimensions match params; bitmap charset
      and length; every claimed dot lies inside the grid.
- [x] 4.3 Property tests: every preset generates a board whose
      solver diagnosis equals the requested difficulty; no
      `AMBIGUOUS`/`IMPOSSIBLE` ever returned to the caller.

## 5. State, UI, input (D4)

- [x] 5.1 `newState(p, desc)`, `newUi(state)`, `newDrawState(state)`.
- [x] 5.2 `interpretMove`: press on or near a dot ⇒ `hold`; drag
...
- [x] 5.3 Immutable `executeMove`: copy typed arrays, mutate copy,
...
- [x] 6.4 `animLength` / `flashLength` / `timingState`.
- [x] 6.5 Recording-`GameDrawing` test: `redraw` only paints inside
...
- [x] 7.1 `textFormat(state)`: plain-text rendering matching
      upstream's `game_text_format`.
- [x] 7.2 `statusbarText(state, ui)`: move count, solved /
      solved-with-help wording, current-puzzle difficulty when
      known.

## 8. Dev-time differential spot-check (D7)

- [x] 8.1 `puzzles/auxiliary/galaxies-trace.c` (transient): emits
...
- [x] 8.2 `scripts/diff-galaxies.test.ts` +
...
- [x] 8.3 `src/native/games/galaxies/__fixtures__/
...
- [x] 8.4 `src/native/games/galaxies/galaxies-differential.test.ts`:
...
- [x] 9.2 Dev-server smoke test: `npm run dev`, exercise all 6
...
- [x] 9.5 `puzzles/CMakeLists.txt`: `puzzle(galaxies TS_PORTED …)`;
      catalog continues to list `galaxies`, no `galaxies.wasm`,
      app routes `galaxies` to the TS engine.
- [x] 9.6 Delete `puzzles/galaxies.c`. Delete
...
- [x] 10.1 `AGENTS.md`: add a "What's been done" entry; mark
      migration-order item 3 landed.
- [x] 10.2 `openspec validate add-galaxies-ts-port --strict` clean.
- [x] 10.3 Full pre-commit gate green (`tsc -b` → biome →
