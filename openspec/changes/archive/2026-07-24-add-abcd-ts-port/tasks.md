...
- [x] 1.1 `scripts/new-game-port.sh abcd` to stamp `src/native/games/abcd/` with
...
- [x] 2.1 `AbcdParams { w, h, n, diag, removenums }`; `encodeParams`/`decodeParams`
      (`%dx%dn%d` + `D` if diag + `R` if removenums, `R` **full-only**; decode reads
      optional `x h`, optional `n n`, optional `D`, optional `R`) — design D2.
- [x] 2.2 `validateParams` in upstream order + messages: `w ≥ 2`, `h ≥ 2`,
      `n ≥ 3` (normal) but `n ≥ 5` when `diag`, `n ≤ 9`.
- [x] 2.3 `paramConfig`: Width, Height, Letters (numeric via `parseConfigInt`),
      "Remove clues" (boolean → `removenums`), "Allow diagonal touching" (boolean →
      `diag = !bval` — port the **inversion**, D2).
- [x] 2.4 Presets — the seven upstream presets (`4×4`/`5×5` Easy+Hard, `6×6` Easy,
...
- [x] 2.5 State (D1): `grid` (`Int8Array`, letter index or `EMPTY`), `pencil`
...
- [x] 2.6 Desc codec (D2): `newGame` parses the comma-separated clue list
      (digits→number, `-`→`NO_NUMBER`); the generator's encoder emits the exact
      inverse. Byte-match surface.
- [x] 2.7 `validateDesc`: per-axis magnitude bound (`num ≤ 1 + w/2` for a row clue,
...
- [x] 3.1 `solveGame(params, numbers)` → discriminated `SOLVED` / `AMBIGUOUS` /
...
- [x] 3.2 `placeLetter(x, y, letter)`: set the cell, rule the letter out of the
...
- [x] 3.3 Technique 1 — satisfied/exhausted clue (`remaining == 0` → rule the letter
...
- [x] 3.4 Technique 3 — runs (`abcd_solver_runs`): per line, maximal candidate runs;
...
- [x] 3.5 Final classification (`validate_puzzle`): contradiction (empty candidate
      set / clue over- or under-satisfied), else incomplete → `AMBIGUOUS`, else all
      cells filled + all clues met → `SOLVED`.
- [x] 3.6 Tier-1 tests: a hand-built tiny puzzle solves to a known grid; a
      contradictory clue set → `CONTRADICTION`; an under-clued set → `AMBIGUOUS`.

## 4. The generator

- [x] 4.1 `generateBoard` (D4): random fill (one `random_upto` per cell over the
...
- [x] 4.2 Remove-clues phase (only when `removenums`): **one `shuffle`** of the
...
- [x] 4.3 `newDesc` emitting the comma-terminated clue list; no `aux` threading —
      `solve()` re-runs `solveGame` from the clues (playbook §3.6, D4).
- [x] 4.4 Tier-1: every shipped preset (bounded attempts) and a small odd-size sweep
...
- [x] 5.1 Move model (D7): the `AbcdMove` discriminated union
      (`enter` / `pencil` / `markAll` / `solve`), not the C move strings.
- [x] 5.2 `Ui` (ephemeral): cursor `hx,hy`, `hpencil`, `hshow`, `hcursor`.
...
- [x] 5.3 Letter entry: `A`–`I` / `a`–`i` / **bare `1`–`9`** (playbook §3.8a — no
...
- [x] 5.4 `executeMove`: `enter` sets the cell (clearing wipes that cell's pencil
...
- [x] 5.6 Win check `isCompleted(state)` = upstream `validate_puzzle == 0` (no clue
      violation, no adjacency violation incl. both diagonals under `diag`, every cell
      filled), D8. `completed` never clears once set.
- [x] 5.7 `textFormat` — the ASCII board (`board_text_format`); `canFormatAsText`
      guarded to `w < 19 && h < 19` (upstream's two-digit-clue limitation).

## 6. `findMistakes` and rendering

- [x] 6.1 `findMistakes(state)` (D5): re-solve the clues to the canonical grid, flag
      ...
      does not.
- [x] 6.2 Palette in C enum order (`COL_OUTERBG`, `COL_INNERBG`, `COL_GRID`,
...
- [x] 6.3 `computeSize`/`setTileSize` (D9): `(w+n)·TILESIZE + 1` × `(h+n)·TILESIZE`,
      `BORDER = 0` (`NARROW_BORDERS` arm), `innerCoord(x) = (x+n)·TILESIZE`.
- [x] 6.4 `redraw`: border letters + edge clues (red on clue error), per-tile letter
...
- [x] 6.5 Live error flags as base render (D5): clue over/under-satisfied
...
- [x] 6.6 Completion flash (D9): diagonal-stripe over `FLASH_TIME = 0.7 s` in
      `FLASH_FRAME = 0.1 s` steps, non-cheated completions only. No move animation
      (`animLength = 0`).
- [x] 6.7 Tier-2.5 render-scenario tests + snapshots: a selected-cursor frame, a
      pencil-mark frame, an adjacency-error frame, and a completion-flash frame.

## 7. Differential

- [x] 7.1 `puzzles/auxiliary/abcd-trace.c` on the established pattern; add its
      `cliprogram()` line.
- [x] 7.2 Fixture matrix (D6): the shipped presets plus a modest odd-biased size
...
- [x] 7.3 `abcd-differential.test.ts`: TS `newDesc` reproduces the C desc
...
- [x] 8.2 Full gate green (`tsc -b --noEmit` → biome → `vitest run` → `vite build`).
- [x] 8.3 `openspec validate add-abcd-ts-port --strict`.
- [x] 8.4 Dev-verify in the browser: select / arrow cursor / ink + pencil entry /
...
- [x] 8.5 Update `docs/porting/game-port-playbook.md` if anything was newly learned
...
- [x] 9.1 Add `TS_PORTED` to the `puzzle(abcd …)` entry in
      `puzzles/unreleased/CMakeLists.txt` (drop `solver(abcd)`).
- [x] 9.2 Delete `puzzles/unreleased/abcd.c`.
- [x] 9.3 `rm -rf build/wasm/` and rebuild — abcd in the catalog, no `abcd.wasm`.
      Icons already exist.
