...
- [ ] 1.1 `scripts/new-game-port.sh sticks` to stamp `src/native/games/sticks/`
...
- [ ] 2.7 `validateDesc`: reject a desc decoding to more/fewer than `w*h` cells
...
- [ ] 3.1 `sticksMakeDsf` + `sticksMaxSizeHorizontal/Vertical` + `sticksValidate`
...
- [ ] 3.2 `sticksTry` (place a trial line; if it makes the board INVALID, commit
...
- [ ] 3.3 Tier-1 tests: a hand-built board solves to its unique solution; an
      already-invalid board reports INVALID; a board needing a deduction step is
      driven to COMPLETE.

## 4. The generator

- [ ] 4.1 `setBlacks` — the `lightup.c`-derived symmetric black placement (region
      ...
      design D3).
- [ ] 4.2 The fill+clue+retry loop: random `F_HOR`/`F_VER` fill, segment dsf, black
      clue counts + per-segment length clue on a `random_upto`-chosen cell, retry
      until `sticksSolveGame == COMPLETE`.
- [ ] 4.3 Greedy minimisation: one `shuffle` of the cell indices, then remove each
      clue keeping it removed only while the board still solves to COMPLETE.
- [ ] 4.4 `newDesc` emitting the run-length desc; no `aux` (solve re-runs the
      solver — playbook §3.6).
- [ ] 4.5 Tier-1: both presets and a small size/`blackpc`/`symm` sweep generate a
      board that `sticksSolveGame` deduces to COMPLETE.

## 5. Input, moves, findMistakes and completion

- [ ] 5.1 Move model: the `SticksMove` discriminated union
      (`{ kind: "set"; changes } | { kind: "solve"; grid }`), not a move string
      (design D4).
- [ ] 5.2 `interpretMove` drag phases (design D4): drag-to-draw
...
- [ ] 5.6 `textFormat` — the board text (`.`/`-`/`|`/`#`); `canFormatAsText` static
      `true` (design D9).

## 6. Rendering

- [ ] 6.1 Palette in C enum order (`BACKGROUND, GRID, LINE, NUMBER, ERROR,
...
- [ ] 6.3 `redraw`: per-tile rendering (black-cell fill, green line bar for
...
- [ ] 6.4 The `findMistakes` error overlay (flagged cells red, `COL_ERROR`) folded
      into the cache word (design D5).
- [ ] 6.5 The completion flash over `FLASH_TIME = 0.5 s` (lines blink off on
      alternate 0.1 s frames). No slide/interpolation — `animLength` is `0`
      (design D8).
- [ ] 6.6 Tier-2.5 render-scenario tests + snapshots: an opening frame, a
      lines+clue frame, a `findMistakes`-flagged frame, and a completion-flash
      frame.

## 7. Differential

- [ ] 7.1 `puzzles/auxiliary/sticks-trace.c` on the established pattern
      (`#include "../unreleased/sticks.c"`); add its `cliprogram()` line.
- [ ] 7.2 Fixture matrix: both presets plus a size/`blackpc`/`symm` sweep
      (design D6), each seed dumping the generated desc.
- [ ] 7.3 `sticks-differential.test.ts`: TS `newDesc` reproduces the C desc
...
- [ ] 8.2 Behavioural tests: codec round-trip, solver on hand-built boards,
...
- [ ] 8.3 Full gate green (`tsc -b --noEmit` → biome → `vitest run` →
      `vite build`).
- [ ] 8.4 `openspec validate add-sticks-ts-port --strict`.
- [ ] 8.5 Dev-verify in the browser: drag-to-draw, left/right click, keyboard
      cursor + Enter/Space, Solve, Check & Save (mistake block + clear), completion
      flash, Custom params (size/blackpc/symmetry).
- [ ] 8.6 Update `docs/porting/game-port-playbook.md` if the port surfaces a new
