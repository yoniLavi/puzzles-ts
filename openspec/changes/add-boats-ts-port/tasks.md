...
- [ ] 1.1 `scripts/new-game-port.sh boats` to stamp `src/native/games/boats/`
      ...
      first.
- [ ] 1.2 Confirm the long-tail-risk checklist against `puzzles/unreleased/boats.c`
...
- [ ] 2.1 `BoatsParams { w, h, fleet, diff, strip, fleetData }`;
...
- [ ] 2.2 `validateParams` in upstream order (`boats.c:2802`): unknown difficulty →
...
- [ ] 2.3 `paramConfig` (Width, Height, Fleet size, Difficulty, Remove numbers,
...
- [ ] 2.4 `describeParams` emits the keys `augmentation.ts` reads — `width`,
...
- [ ] 2.5 Presets — the twelve upstream presets (`boats.c:178`–`189`: 6×6/8×8 ×
      Easy/Normal/Hard, 10×10 × all four, 10×12 × Tricky/Hard), each with its fleet
      size and default fleet.
- [ ] 2.6 `BoatsCell` union (`EMPTY`/`WATER`/`SHIP_VAGUE`/`SHIP_SINGLE`/`SHIP_TOP`/
      ...
      `cloneState`.
- [ ] 2.7 Desc codec (design D1): decode = border clues (`w+h` `"N,"`/`"-,"` tokens)
...
- [ ] 2.8 `validateDesc`: unknown character, wrong number of grid squares
      (distinguish too-much/too-little), wrong number of border-clue slots
      (`boats.c:479`).

## 3. The four-tier deductive solver (design D2)

- [ ] 3.1 `solveBoats(state, maxDiff)` → reached `Difficulty` or `IMPOSSIBLE`
...
- [ ] 3.2 Easy rung: `boats_solver_initial`, `boats_solver_check_fill`,
      `boats_solver_check_counts`, `boats_solver_centers_trivial`,
      `boats_adjust_ships` (diagonal→water + segment-shape resolution).
- [ ] 3.3 Normal rung (over the shared [`Dsf`](../../src/native/engine/dsf.ts) — design
...
- [ ] 3.4 Tricky rung: `boats_solver_shared_diagonals`, and the missing-border-clue
      deduction `boats_solver_borderclues_fill`/`_last` (fill, solve, restore).
- [ ] 3.5 Hard rung: the scratch-grid hypothesis techniques
      `boats_solver_attempt_ship_rows` / `_attempt_water_rows` /
      `boats_solver_centers_attempt`.
- [ ] 3.6 Tier-1 tests: a hand-built board solves at each tier; a contradictory
      board returns `IMPOSSIBLE`; the `dsf`-root-reading `boats_check_dsf` deduction
      is exercised.

## 4. The generator (design D6)

- [ ] 4.1 `boatsGenerateFleet(state, rs, …)` (`boats.c:2705`) — random fleet
      placement; RNG draws reproduced in order (byte-match).
- [ ] 4.2 `boatsCreateBorderclues` (`boats.c:2866`) — derive the `w+h` edge counts.
- [ ] 4.3 `newDesc`: solver-gate to **exactly** the target difficulty
...
- [ ] 4.4 Tier-1: every preset + a size/fleet sweep + both `strip` states generate a
      board solvable at exactly its target difficulty via a fresh `solveBoats`.

## 5. Input, moves, win and mistakes

- [ ] 5.1 Move model: `BoatsMove` discriminated union
      (`{ kind: "fill"; x0,y0,x1,y1; from; to } | { kind: "solve"; grid }`), not the
      `P…` string (design D4).
- [ ] 5.2 `interpretMove` (`boats.c:3203`): left-click cell-cycle (empty→ship→water),
...
- [ ] 5.3 Keyboard: cursor (arrows), `CURSOR_SELECT`/`\b` ship, `CURSOR_SELECT2`
      water, Ctrl/Shift+arrow line-fill (`boats.c:3300`+).
- [ ] 5.4 `executeMove`: apply the line fill, then `boats_adjust_ships`
...
- [ ] 5.5 `findMistakes` (design D5): **re-solve to the unique solution** and flag
...
- [ ] 5.6 `solve()` re-runs `solveBoats(_, Hard)` and returns `{ kind: "solve", grid }`;
      error on insoluble. Test Solve **through a real `Midend`** (playbook §3.6).
- [ ] 5.7 `textFormat` — `board_text_format` (`boats.c:572`); widen to `string`,
      static `canFormatAsText: true`.

## 6. Rendering (design D9)

- [ ] 6.1 Palette in `COL_*` enum order (`boats.c:52` — 15 colours, water at index 4
...
- [ ] 6.2 `computeSize`/`setTileSize`: the **`NARROW_BORDERS`** arm (`BORDER = 0`);
      width `(w+1)*TILESIZE`, height `GUTTER + (h+1+fleetHeight)*TILESIZE − padding`
      (`boats.c:3944`).
- [ ] 6.3 `redraw`: per-tile piece rendering (water, six segment shapes + single +
...
- [ ] 6.4 Live error overlays (`COL_SHIP_ERROR`/`COL_COUNT_ERROR`/
      `COL_COLLISION_ERROR`) drawn from the `findMistakes` set (design D5), and the
      cursor (`COL_CURSOR_A/B`).
- [ ] 6.5 The completion flash over `FLASH_TIME` on the not-completed→completed
      transition. No move interpolation — `animLength` is `0`.
- [ ] 6.6 Tier-2.5 render-scenario tests + snapshots: a fresh board with clues + fleet,
...
- [ ] 7.1 `puzzles/auxiliary/boats-trace.c` `#include`ing `../unreleased/boats.c`;
      add its `cliprogram(boats-trace boats-trace.c)` line. Build pure-C
      (`-DUSE_TS_RANDOM=0`, playbook §4.2).
- [ ] 7.2 Fixture matrix: every preset, both `remove-numbers` states, and a
      size/fleet sweep — each seed dumping the generated desc.
- [ ] 7.3 `boats-differential.test.ts` via
...
- [ ] 8.2 Behavioural tests: params/desc round-trip, solver per tier, generator
...
- [ ] 8.3 Full gate green (`tsc -b --noEmit` → biome → `vitest run` → `vite build`).
- [ ] 8.4 `openspec validate add-boats-ts-port --strict`.
- [ ] 8.5 Dev-verify in the browser: click-cycle, right-click water, drag-fill a
