...
- [ ] 1.1 `scripts/new-game-port.sh clusters` to stamp `src/native/games/clusters/`
...
- [ ] 2.1 `ClustersParams { w, h }`; `encodeParams`/`decodeParams` (`%dx%d`, with
      `h = w` fallback on a bare number — via the shared dimension parser).
- [ ] 2.2 `validateParams` in upstream order: `w*h < 10000` ("Puzzle is too
      large") → `w*h < 2` ("Puzzle is too small").
- [ ] 2.3 `paramConfig` (Width, Height) — keys matching the C config slugs
      (playbook §3.4); numeric `set` via `parseConfigInt`.
- [ ] 2.4 Presets — the four upstream presets (`7×7`, `8×8`, `9×9`, `10×10`);
      `describeParams` emits the keys `augmentation.ts` reads (playbook §3.4).
- [ ] 2.5 State: the grid as a `Uint8Array` of the cell flag byte
...
- [ ] 2.6 Desc codec (design D1): the run-length dot encoding — `a`–`y` red dot,
...
- [ ] 2.7 `validateDesc`: reject accumulated position `< s+1` ("too short") vs
      `> s+1` ("too long") distinctly, and any character outside `a`–`z`/`A`–`Z`.

## 3. The contradiction solver (design D2)

- [ ] 3.1 `clustersValidate(state)` → discriminated `COMPLETE | UNFINISHED |
...
- [ ] 3.2 `solverTry` (single-cell forced-colour by contradiction) and
...
- [ ] 3.3 Record in `solver.ts`'s module doc that the solver is contradiction-based
...
- [ ] 3.4 Tier-1 tests: a hand-built board solves to a known unique fill;
...
- [ ] 4.1 `clustersGenerate(state, temp, rng, force)`: per-cell two-colour fill,
...
- [ ] 4.2 `newDesc`: loop `clustersGenerate` until `COMPLETE`, forcing a full
      ...
      playbook §3.6).
- [ ] 4.3 Tier-1: every preset and a small size sweep generate a board whose desc
      decodes to a state the TS solver completes to a unique fill.

## 5. Moves, input and completion

- [ ] 5.1 Move model: the `ClustersMove` discriminated union
      (`{ kind: "paint"; cells } | { kind: "solve"; grid }`), not a move string
      (design D4).
- [ ] 5.2 `interpretMove` mouse paint-drag (design D9): `LEFT_BUTTON` cycles the
      ...
      (`BORDER` offset, D7).
- [ ] 5.3 `interpretMove` keyboard (design D9): arrows move the cursor
...
- [ ] 5.4 `executeMove`: apply each cell edit (**never** overwriting an
...
- [ ] 5.6 `findMistakes` (design D5): return the `clustersValidate` `INVALID` cell
...
- [ ] 5.7 `textFormat` — the board text (`r`/`b`/`.`, uppercase for givens);
      `canFormatAsText` static `true` (design D8).

## 6. Rendering (design D7)

- [ ] 6.1 Palette in C enum order (`BACKGROUND, GRID, COL_0, COL_1, COL_0_DOT,
...
- [ ] 6.3 `redraw`: per-tile rendering (grid rect + colour rect, dot circle, error
...
- [ ] 6.4 The completion flash over `FLASH_TIME = 0.3 s` inverting both colours on
      even half-frames; no move animation (`animLength` = 0 — design D7).
- [ ] 6.5 Tier-2.5 render-scenario tests + snapshots: a fresh board (dots drawn), a
      mid-paint drag frame, a `findMistakes` error-outline frame, and a completion-
      flash frame.

## 7. Differential (design D6)

- [ ] 7.1 `puzzles/auxiliary/clusters-trace.c`
...
- [ ] 7.2 Fixture matrix: every preset plus a size sweep, each seed dumping the
      generated desc.
- [ ] 7.3 `clusters-differential.test.ts`: TS `newDesc` reproduces the C desc
...
- [ ] 8.2 Behavioural tests: codec round-trip, solver classification + forced-colour
...
- [ ] 8.3 Full gate green (`tsc -b --noEmit` → biome → `vitest run` →
      `vite build`).
- [ ] 8.4 `openspec validate add-clusters-ts-port --strict`.
- [ ] 8.5 Dev-verify in the browser: paint-drag (both colours, cycling), keyboard
      cursor + place keys, Solve, completion flash, Check & Save mistake highlight,
      Custom params. 0 console errors.
- [ ] 8.6 Update `docs/porting/game-port-playbook.md` if the paint-drag input or the
