...
- [ ] 1.1 Reproduce in the running app: open any game with a press-driven
...
- [ ] 1.2 Confirm the game is not at fault: a press then a release through a
...
- [ ] 2.1 Retain a `pointerup` / `pointercancel` that arrives while the press's
      ...
      await.
- [ ] 2.2 Guarantee exactly-once delivery: never zero releases (the bug), never
      two (a replayed release plus a real one). Keep the declined-press path's
      immediate release unchanged.
- [ ] 2.3 Check the `pointercancel` and `cancelPointerTracking` paths for the
      same window, and the touch path (`detectSecondaryButton` may already have
      consumed the up).

## 3. Test and verify

- [ ] 3.1 Tier-3 regression test (`happy-dom`): dispatch `pointerdown` then
...
- [ ] 3.2 Browser-verify on two drag games (Spokes and Pegs): a fast click
      leaves no stranded overlay, and an ordinary press-drag-release still
      works, on both the TS and C/WASM paths.
- [ ] 3.3 Full gate green; `openspec validate fix-click-release-race --strict`.
