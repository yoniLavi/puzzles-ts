...
- [ ] 1.1 `scripts/new-game-port.sh spokes` to stamp `src/native/games/spokes/`
...
- [ ] 1.2 Confirm the long-tail-risk checklist (design intro): no `supersededDesc`,
...
- [ ] 2.1 `SpokesParams { w, h, diff }` (`diff` = Easy/Tricky/Hard);
      `encodeParams` (`%dx%d` then `d<e|t|h>` when full) / `decodeParams` (w, optional
      `x`h with square fallback, optional `d<char>`).
- [ ] 2.2 `validateParams`: `w ≥ 2`, `h ≥ 2` (no upper bound — faithful to the C).
- [ ] 2.3 `paramConfig` (Width, Height string items + a Difficulty choices item);
      keys matching the C config slugs — `width` / `height` / `difficulty` (playbook
      §3.4), numeric `set` via `parseConfigInt`.
- [ ] 2.4 Presets — the six upstream presets (`4×4` and `6×6`, each Easy/Tricky/Hard;
...
- [ ] 2.5 State: `numbers` (`Int8Array`, `0` = hole) and `spokes` (`Uint16Array`,
...
- [ ] 2.6 Desc codec: flat one-char-per-cell grid of clue digits (`'0'`–`'8'`, `'X'`
      for a hole) — **not** run-length (D1). `newState` reads digits → clues and
      processes holes.
- [ ] 2.7 `validateDesc`: each char `'0'`–`'8'` or `'X'`; distinguish "too short"
      (< `w*h`) from "too long" (trailing data) — faithful to the C.

## 3. The tiered deductive solver (design D2, D3)

- [ ] 3.1 The recount scratch (`nodes` / `lines` / `marked` / `dsf` / `open`) as an
...
- [ ] 3.2 `spokesValidate` → `INVALID` / `INCOMPLETE` / `VALID` (discriminated, not
      magic `0/1/2`): per-hub line/mark counts, crossing-`LINE`-diagonal check, and
      the `dsf`-based isolated-set / fully-solved check.
- [ ] 3.3 The deduction rules: `spokesSolverOnes`, `spokesSolverFull`,
...
- [ ] 3.4 Tier-1 tests: a hand-built board solves to a known unique solution; an
...
- [ ] 4.1 `spokesGenerateHubs` — draw every H/V line and one random diagonal per
...
- [ ] 4.2 The strip loop: for each shuffled line, tentatively remove it (keep every
      ...
      unless Easy).
- [ ] 4.3 `newDesc`: loop `spokesGenerate` until it accepts, then emit the flat digit
      desc. No `aux` (the solver re-derives — playbook §3.6).
- [ ] 4.4 Tier-1: every preset and a small size sweep generate a board whose clues
...
- [ ] 5.1 Move model: the `SpokesMove` discriminated union (`{ kind: "set"; index;
      dir; state } | { kind: "solve"; spokes }`), not a move string.
- [ ] 5.2 `interpretMove` drag phases: press records `drag_start` on the ephemeral
...
- [ ] 5.3 Keyboard: arrow-key cursor on the `(3w−2)×(3h−2)` half-grid; `CURSOR_SELECT`
      draws, `CURSOR_SELECT2` marks. Reuse `engine/pointer.ts` cursor helpers where
      they fit.
- [ ] 5.4 `executeMove`: apply a `set` via `spokesPlace` (respecting `HIDDEN`); apply
...
- [ ] 5.5 `solve()` returns `{ kind: "solve", spokes }` from a full-difficulty
      `spokesSolve` of a clean copy; error when no solution is found. Test Solve
      **through a real `Midend`** (playbook §3.6).
- [ ] 5.6 `textFormat` — the ASCII board grid (`game_text_format`); `canFormatAsText`
      stays static `true` (works on any `w×h`).

## 6. `findMistakes` (design D6)

- [ ] 6.1 `findMistakes(state)`: re-solve a clean (clues-only) copy to the unique
...
- [ ] 6.2 Keep it distinct from the existing live `COL_ERROR` rendering (over-count /
      isolated set) — that stays as immediate local validation, separate from the
      solution comparison.
- [ ] 6.3 Paint-twice tier-2 test (playbook §3.2): draw a wrong line, `findMistakes()`,
...
- [ ] 7.1 Palette in C enum order (`BACKGROUND, BORDER, HOLDING, LINE, MARK, DONE,
      ERROR, CURSOR`). Derive from the app background; do **not** luminance-adjust for
      dark mode (playbook §3.3 — the app owns it).
- [ ] 7.2 `computeSize`/`setTileSize`: `w*TILESIZE × h*TILESIZE`, `BORDER = 0`, hubs
      centred at `TOCOORD` (D7).
- [ ] 7.3 `redraw`: per-hub rendering (circle + eight spoke dots, `LINE` spokes as
...
- [ ] 7.4 The keyboard-cursor blitter overlay driven off the ephemeral `Ui` cursor.
- [ ] 7.5 The completion flash over `FLASH_TIME = 5 × FLASH_FRAME = 0.6 s`. No slide
      interpolation — `animLength` is `0`.
- [ ] 7.6 Tier-2.5 render-scenario tests + snapshots: a mid-solve frame, a drag-hold
      frame, a `findMistakes` frame, and a completion-flash frame.

## 8. Differential (design D9)

- [ ] 8.1 `puzzles/auxiliary/spokes-trace.c` on the established pattern
      (`#include "../unreleased/spokes.c"`); add its `cliprogram()` line. Dumps the
      generated desc for `(w, h, diff, seed)` tuples.
- [ ] 8.2 Fixture matrix: every preset plus a small size sweep, each seed dumping the
      generated desc.
- [ ] 8.3 `spokes-differential.test.ts` via `describeDescDifferential`: TS `newDesc`
...
- [ ] 9.2 Behavioural tests: codec round-trip, solver tiers, generator determinism,
      move/toggle transitions, diagonal-crossing rejection, win condition, midend
      lifecycle + save round-trip.
- [ ] 9.3 Full gate green (`tsc -b --noEmit` → biome → `vitest run` → `vite build`).
- [ ] 9.4 `openspec validate add-spokes-ts-port --strict`.
- [ ] 9.5 Dev-verify in the browser: left-drag lines, right-drag marks, diagonal
