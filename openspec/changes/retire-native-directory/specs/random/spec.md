# random Specification Delta — retire-native-directory
...
**Reason**: The requirement specifies an Emscripten `--js-library` bridge — seven
`random_*` symbols delegating to `Module.tsRandomBridge`, a handle table mapping
C integers to TS `RandomState`s, `_malloc`/`stringToUTF8` for
`random_state_encode`'s return. Every one of those mechanisms was deleted by
`retire-c-engine`: there is no WASM engine, no `random_bridge.js`, no
`src/native/random/bridge.ts`, and no C caller. `retire-c-engine` removed the
*flag* that selected between C and TS but left the requirement describing the
bridge the flag selected, which reads as a live obligation to maintain a shim
for a caller that does not exist.

**Migration**: None. The TypeScript implementation is reached by direct import
from the engine and the games. The bit-identical guarantee that made the bridge
interesting is untouched and still asserted by `random.test.ts` against the
frozen corpus.

### Requirement: Pre-commit hook enforces type-check, lint, and tests

**Reason**: Misfiled and three steps out of date. It requires `.husky/pre-commit`
to run `tsc -b --noEmit`, `npm run lint` and `npm run test:run`; the real gate
has run five steps since — `tsc` → biome (lint **and** format **and** import
order) → the probe-anchor check → `vitest run` → `vite build` — and that gate is
specified properly by `build-pipeline`'s "The pre-commit gate minimises
wall-clock without dropping checks", including why each step is there. Two specs
describing one gate means one of them is wrong, and this is the wrong one. It
landed in the `random` capability only because `port-random-to-typescript`
happened to be the change that introduced the hook.

**Migration**: None. `build-pipeline` already carries the requirement, with the
current step list and the reasoning for it.
