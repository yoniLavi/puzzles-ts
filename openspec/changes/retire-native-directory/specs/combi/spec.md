...
- FROM: `### Requirement: TypeScript combi module reproduces C output byte-for-byte`
- TO: `### Requirement: TypeScript combi module enumerates subsets in lexicographic order`
...
**Reason**: The corpus recorded the lex-ordered subsets of `{0, …, n-1}` for five
`(r, n)` pairs. That is not a fact about upstream — it is what the definition of
a combination requires — so the recording could only ever confirm that C and
TypeScript both implement combinations correctly. The module's own test file
already asserted every closed-form property exhaustively over `n ≤ 8`: exactly
`C(n, r)` tuples, strictly ascending, strict lex order, all distinct, all drawn
from `{0..n-1}`. Those strictly dominate a replay of five pairs, so the corpus
was carrying a `__fixtures__/` directory, a spec requirement and a "frozen
oracle, never re-baseline" rule for no assurance the file did not already have.

This is deliberately **not** a precedent for the 48 per-game differentials or
`random`'s corpus: those record facts that cannot be derived from a definition,
which is exactly why they stay.

**Migration**: The properties in `combi.test.ts` are the requirement, now stated
as "Enumeration correctness is asserted in closed form, not by replay". The one
behaviour the corpus block alone exercised — `reset()` — is now a direct test,
and the "reset rewinds the iterator" scenario is unchanged.
