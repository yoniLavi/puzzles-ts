...
- [ ] 1.1 `scripts/new-game-port.sh salad` to stamp `src/native/games/salad/` with
...
- [ ] 2.1 `SaladParams { order, nums, mode, diff }`; `encodeParams`/`decodeParams`
      (`%dn%d%c` — order, `n`, nums, `L`/`B` mode — then `d%c` diff char on full;
      `decode` reads a leading int, optional `n<nums>`, `L`/`B`, optional `d<char>`).
- [ ] 2.2 `validateParams` in upstream order: `nums ≥ 2` → `nums < order` →
      `order ≥ 3` → `nums ≤ 9` → `diff` in range, with the C messages.
- [ ] 2.3 `paramConfig` (Game Mode / Size (s*s) / Symbols / Difficulty) with keys
...
- [ ] 2.4 Presets — the eleven upstream presets (design D5 table); labels
      `Letters: NxN A~c` / `Numbers: NxN 1~c`.
- [ ] 2.5 State: `SaladState { grid, holes, borderclues, gridclues, marks,
...
- [ ] 2.6 Desc codec (design D4): the shared run-length `salad_serialize` (empty-run
...
- [ ] 2.7 `validateDesc` = decode-and-discard, reproducing the C messages:
...
- [ ] 3.1 `usersolver` (upstream `salad_solver_easy`): `latinholes_solver_sync`
...
- [ ] 3.2 Difficulty driver: **Normal** (`DIFF_EASY`) and **Extreme** (`DIFF_HARD`)
...
- [ ] 3.3 `latinholes_check` (line hole counts + each symbol once) and
      `salad_checkborders` (letters mode) as the completion predicate.
- [ ] 3.4 `salad_solve(state, maxdiff)` — place fixed clues into the cube, run
...
- [ ] 5.3 Symbol / `X` / `O` / Backspace entry in ink or pencil mode, honouring the
      C's guards (can't place on a fixed number clue; circle-clue restrictions).
- [ ] 5.4 `M` mark-all (fill every empty cell's candidates; `canMarkAll: true`,
      playbook §3.7) — emitted only when it would change the board.
- [ ] 5.5 `executeMove`: apply `set`/`pencil`/`markAll`/`solve` immutably; recompute
      `completed` via `latinholes_check` + `salad_checkborders`; `solve` sets
      `completed = cheated = true`.
- [ ] 5.6 `solve()` re-runs `salad_solve(dup, DIFF_HARD)`; returns the solved grid as
      a `{ kind: "solve" }` move, or errors "No solution found." Test Solve **through
      a real `Midend`** (playbook §3.6).
- [ ] 5.7 `textFormat` — the ASCII board (`game_text_format`); `canFormatAsText`
      stays static `true`.
- [ ] 5.8 `requestKeys` (playbook §3.8): `nums` symbol keys (base `A`/`1`), then `X`,
...
- [ ] 6.2 `computeSize`/`setTileSize`: `(order + 2)² · tilesize`, one-tile clue
      margin (`FROMCOORD = x/TILE_SIZE − 1`) — not `NARROW_BORDERS`.
- [ ] 6.3 `redraw`: per-tile cell (bg / cursor / pencil-triangle), square border,
...
- [ ] 6.4 Live error highlight (duplicate symbol in a line; border-clue violation) as
      its own flag bits — distinct from `findMistakes` (design D8).
- [ ] 6.5 Completion flash over `FLASH_TIME = 0.7 s`, three-phase diagonal wave
      (`(int)(flashtime / FLASH_FRAME) % 3`). `animLength` is `0`.
- [ ] 6.6 Tier-2.5 render-scenario tests + snapshots: selected ink cell, pencil-marked
      cell, ball/cross clue, live error, `findMistakes` overlay, completion flash.

## 7. `findMistakes` + pencil UX

- [ ] 7.1 `findMistakes(state)` (playbook §3.5): re-solve from fixed clues to the
...
- [ ] 7.2 Pencil UX (playbook §3.7): `canMarkAll` (done in 5.4), sticky-pencil `Ui`
      pref via the `prefs` hook, a pencil-mode indicator, notes-first-class in
      `findMistakes` (7.1).
- [ ] 7.3 Paint-twice mistake test (§3.2): paint, `findMistakes()`, redraw the same
      drawstate, assert the highlight appears on the second paint and clears on a
      third overlay-free frame.

## 8. Differential

- [ ] 8.1 `puzzles/auxiliary/salad-trace.c` on the established pattern
      (`#include "../unreleased/salad.c"`, linking `latin.c`); add its `cliprogram()`
      line. Dumps the generated desc for `(params, seed)` tuples.
- [ ] 8.2 Fixture matrix: every preset × both difficulties (design D9), plus a size
      sweep, each seed dumping the generated desc.
- [ ] 8.3 `salad-differential.test.ts`: TS `newDesc` reproduces the C desc
      byte-for-byte across all fixtures (validates generator + solver + codec).

## 9. Registration and stage-1 close-out

- [ ] 9.1 Register in `ts-ported-ids.ts` + `games/index.ts` (TS-served). C/WASM
      remains the fallback — no CMake change yet (`salad.c` still builds `salad.wasm`).
- [ ] 9.2 Behavioural tests: params/desc round-trip, solver uniqueness, generator
      determinism (same seed → same desc), move transitions, win detection, midend
      lifecycle + save round-trip.
- [ ] 9.3 Full gate green (`tsc -b --noEmit` → biome ci → `vitest run` →
      `vite build`).
- [ ] 9.4 `openspec validate add-salad-ts-port --strict`.
- [ ] 9.5 Dev-verify in the browser: both modes, letter/number entry, pencil marks,
      mark-all, `X`/`O` markers, keypad, Check & Save (clean + mistake), Solve,
      completion flash, Custom params.
- [ ] 9.6 Update `docs/porting/game-port-playbook.md` if the port surfaces anything
