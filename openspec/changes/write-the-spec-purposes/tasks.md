...
- [ ] 1.1 List the placeholder specs with the query, not the proposal's count:
...
- [ ] 2.1 For each spec, read its requirement headings (and the change named in
...
- [ ] 2.2 Game specs share a shape, so keep their Purposes parallel without making
...
- [ ] 2.3 Batch commits by whatever keeps a diff readable. Verify each by shape:
      every changed line in the diff is inside a `## Purpose` section, and no
      requirement or scenario line moved.

## 3. Upgrade openspec

- [ ] 3.1 Check the current openspec release and read its changelog from 1.11.0
      ...
      one.
- [ ] 3.2 `npm install -D @fission-ai/openspec@^<latest>`, then
...
- [ ] 3.3 `AGENTS.md` names no openspec version number (it points at
