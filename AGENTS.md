...
2. **Pattern-establishing game** — **Landed** (`add-flip-ts-port`): **Flip** ported end-to-end through the new midend (first `registerGame(...)`, `flip.c` deleted, `TS_PORTED` catalog seam, the deferred drawing/UI-feedback/colour refinements resolved, `PuzzleEngineSurface` replacing the dispatch cast, advisory + frozen differential checks). The pattern for subsequent ports is now established.
