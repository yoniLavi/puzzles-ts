...
2. **Pattern-establishing game** — **Flip ported and at owner-confirmed parity 2026-05-20** (`add-flip-ts-port`). Shipped broken: midend didn't repaint, no animation. Then took three iterations on the rendering pipeline — each surfaced a distinct real defect: midend-doesn't-repaint (`5c5eba4`), animated-first-frame flicker (`b7dc206`), then `9823acd` mirrored `midend.c`'s first-draw too literally and caused ResizeController-driven cache wipes; `b49bfdb` reshaped that into a side-effect-free `Midend.size` + explicit `canvasCleared` and moved the bg fill into each game's `!ds.started` branch (engine emits no pixels of its own); finally `b1b0dd6` fixed a state-machine bug where `Midend.timer` accumulated `flashTime` unconditionally, causing Flip's solve-celebration flash overlay to fire on every animated move. The reusable port pattern (engine contract, `TS_PORTED` catalog seam, differential checks) stands. Lessons spec-enforced: green suite ≠ parity (`add-parity-gated-registration`); the engine paints no pixels of its own and `Midend.size` is informational (`fix-flip-canvas-reshape`); motivates the next step — the scene-graph `Game.scene()` contract (direction approved 2026-05-20; design archived as `2026-05-20-scaffold-scene-graph-game-contract`; implementation in flight as `add-scene-graph-reconciler` with Flip-rewrite as pilot) where the framework reconciles and games are pure state→scene functions, eliminating the per-game-cache fragility that bit us three times here.
3. **Scene-graph rendering contract** — **next, in flight** (`add-scene-graph-reconciler`). Lands the declarative `Game.scene()` contract: a small in-repo reconciler (~200 lines) that diffs per-frame `SceneNode[]` trees by stable id and emits the minimum canvas writes; games become pure state→scene functions with no per-tile cache to mis-invalidate. **Pilot = Flip-rewrite**, not Galaxies — Flip's owner-confirmed visual + behavioural parity is the strongest possible baseline to validate the new contract against, and the change that motivated the design is the change that proves it. After this lands, Galaxies and every subsequent port may start scene-graph-native; the per-tile-cache class of bugs becomes structurally impossible. `redraw` stays optional indefinitely (any future port may pick either path); deprecation deferred until every shipped port has migrated.
4. **Galaxies** — the goal-4 game, scene-graph-native from the start (lands on the contract step #3 establishes); once TS, the cell↔dot aid is a small follow-up.
5. **Cross-game features** — quick-save (app-shell, small), optional `findMistakes()` / `hint()` hooks on the `Game` interface implemented per-game as games land.
6. **Outward** — remaining games, simplest-first; leaf libs pulled in idiomatically as needed; worker existence re-evaluated once games are TS (it exists for heavy WASM; light TS games may not need it).
7. **`random.c`** is already TS (`random.ts`); keep it.
