...
- **The gate is not what gets trimmed.** `tsc` → biome → probe-anchor → spelling → `openspec validate` → `vitest run` → `vite build` stays, and NEVER bypass it (point 4 above is unconditional). What gets trimmed is *what vitest runs*, not whether it runs.
...
- **A slow tier nobody invokes is not coverage.** `npm run test:slow` is a once-per-refactoring-round check, so a single file that takes tens of minutes has not been made thorough, it has been made unrunnable — and the honest fix is a cheaper configuration, not a longer wait. Measured 2026-09-08: `hint-resume.test.ts` at five seeds × every preset ran 50 minutes without reporting and was cut to two.
