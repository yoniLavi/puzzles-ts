...
- `npm run build:wasm` — compiles the puzzle wasm + manual into `src/assets/puzzles/` via `scripts/build-emcc.sh`. Honours `USE_TS_LEAVES=1` (the primary "give me the hybrid TS+C build" toggle — must be paired with `VITE_USE_TS_LEAVES=1` for vite) and the per-module `USE_TS_<MODULE>` overrides (today: `USE_TS_RANDOM`; future: `USE_TS_COMBI`, …) which can flip an individual seam against the umbrella. Per-module Vite env vars (`VITE_USE_TS_RANDOM`, …) similarly override `VITE_USE_TS_LEAVES`. The worker fails closed at WASM instantiation if the CMake and Vite flag sets disagree (`assertWasmBridgesCoherent` in `src/puzzle/worker.ts`). When *transitioning* between flag combinations (e.g. you previously cached `USE_TS_RANDOM=ON` and now want a clean default-OFF build), reset cmake's cache with `rm -rf build/wasm/` before the next `npm run build:wasm` — cmake's `option()` honours previously-cached values, so a stale cache will silently win.
