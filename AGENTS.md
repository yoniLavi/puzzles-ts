...
4. **Helper-extraction prep change** (do before port #3) — extract `src/native/engine/{colour-mkhighlight,pointer,dsf}.ts` based on the post-Galaxies evaluation. See "Helper extractions queued before next port" below.
5. **Pattern-validation port** — a *small* game next (Pegs / Cube / Unequal, ~1k C lines), not Loopy/Solo. Locks in the helper-extraction template before the next big game.
6. **Cross-game features** — quick-save (app-shell, small), optional `findMistakes()` / `hint()` hooks on the `Game` interface implemented per-game as games land.
...
## Helper extractions queued before next port

The post-Galaxies evaluation identified three shared-helper extractions that compound across the long tail (~30 ports ahead). They were *deliberately deferred* out of the Galaxies change to keep its scope coherent, but should land in a small prep change before port #3, validated by porting a small game (Pegs / Cube / Unequal) on top of them. Doctrine permits this — the project's no-mid-migration-architecture-pivot rule (memory `feedback_dont_interleave_arch_pivot_with_migration`) is about engine-contract pivots like scene-graph; these are pure leaf-helper additions with no contract change. Each promotion is gated on the second-consumer rule the Galaxies design.md D1 spelled out for `dsf`.

1. **`src/native/engine/colour-mkhighlight.ts`** — port `misc.c`'s `game_mkhighlight_specific` once, with the epsilon fix from `add-galaxies-ts-port`. Every white/black-tile game will want it (Solo, Loopy, Map, Pattern, Range, …). One copy with one fix, not three.

2. **`src/native/engine/pointer.ts`** — button code constants (`LEFT_BUTTON`, `RIGHT_DRAG`, etc., currently re-declared per-game) plus a `PointerSession` helper that lets `interpretMove` consume typed press / drag / release events instead of a raw button number switch. Galaxies is the first drag-and-hold game; Loopy / Slant / Untangle / Pearl / Net all want variations. The "deliberately not handled" cases (Galaxies ignores `LEFT_DRAG`/`LEFT_RELEASE`) become a discriminated case the compiler tracks.

3. **`src/native/engine/dsf.ts`** — promote the local Galaxies `Dsf` class to engine-level on the next dsf-using game (Loopy / Slant / Tents / Magnets are all candidates). The promotion itself is the import-path move + maybe one or two extra methods (`canonifyWithSize`, `minimal`) when actually needed. Phantom API surface stays small.

Also queued, not blocking, but worth knowing:

- **Cache-key shape**: the next port's `render.ts` should use the packed-bits-in-`Int32Array` pattern Galaxies now uses (post-cleanup) — *not* `BigInt64Array`. Document in the next port's design.md.
- **Per-puzzle icons**: 30 more games means 60 more hand-produced PNGs per the `puzzle-icons` spec. A `--screenshot` dev-URL query param would amortise that. Cheap, do before port #4.
