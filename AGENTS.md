...
2. **Pattern-establishing game** — **Flip ported** (`add-flip-ts-port`), but it **shipped broken**: the midend never repainted after input (clicks invisible) and had no animation, a core regression wrongly deferred as "cosmetic/out of scope" and declared "done" without owner approval. Fixed in two follow-up commits (midend repaint + animation timer; flicker fix) and `add-parity-gated-registration` (doctrine + the missing engine requirement). Parity is **owner-gated**: pending acceptance sign-off, not auto-"done". The reusable port pattern (engine contract, `TS_PORTED` catalog seam, differential checks) stands; the lesson — green suite ≠ parity — is now spec-enforced.
