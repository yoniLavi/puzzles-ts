...
- **A slow tier nobody invokes is not coverage.** `npm run test:slow` is a once-per-refactoring-round check, so a single file that takes tens of minutes has not been made thorough, it has been made unrunnable — and the honest fix is a cheaper configuration, not a longer wait. Measured 2026-09-08 on an idle machine: `hint-resume.test.ts` walking every preset once is **776 s**, and it was five seeds deep when the question was asked.
