...
6. **Cross-game features** — the `hint()` hook **shipped** (`add-hint-system` 2026-05-27, deepened to plan-carrying hints by `add-hint-plans` 2026-06-10; Sixteen is the first implementer). Quick-save and `findMistakes()` remain open.
...
- **Per-puzzle icons**: ~30 more games means ~60 more hand-produced PNGs per the `puzzle-icons` spec. A `--screenshot` dev-URL query param would amortise that. Cheap; still outstanding (was "do before port #4" — ports #3 and #4 shipped without it, so it is now overdue, do before port #5).
